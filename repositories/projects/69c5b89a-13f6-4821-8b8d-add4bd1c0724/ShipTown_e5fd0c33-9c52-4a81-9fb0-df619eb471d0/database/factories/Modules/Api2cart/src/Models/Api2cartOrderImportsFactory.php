<?php

namespace Database\Factories\Modules\Api2cart\src\Models;

use App\Modules\Api2cart\src\Models\Api2cartOrderImports;
use Illuminate\Database\Eloquent\Factories\Factory;

class Api2cartOrderImportsFactory extends Factory
{
    protected $model = Api2cartOrderImports::class;

    public function definition(): array
    {
        // this is a real example of the response from the API, copied raw

        return [
            'api2cart_order_id' => $this->faker->unique()->randomNumber(),
            'raw_import' => function () {
                return $this->getDefaultRawImport();
            }
        ];
    }

    /**
     * Configure shipping method.
     *
     * @param string $shippingMethod
     * @return $this
     */
    public function withShippingMethod(string $shippingMethod)
    {
        return $this->state(function (array $attributes) use ($shippingMethod) {
            return [
                'raw_import' => $this->getRawImportForShippingMethod($shippingMethod),
            ];
        });
    }

    /**
     * Get raw import data for specific shipping method.
     *
     * @param string $shippingMethod
     * @return array
     */
    private function getRawImportForShippingMethod(string $shippingMethod): array
    {
        if ($shippingMethod === 'InPost') {
            return self::getInPostRawImport();
        } elseif ($shippingMethod === 'DPD') {
            return self::getDpdRawImport();
        }

        return $this->getDefaultRawImport();
    }

    public function getDefaultRawImport()
    {
        return json_decode('
{
  "id": "1000365271",
  "total": {
    "total": 37.94,
    "total_tax": 0,
    "total_paid": 37.94,
    "total_discount": 0,
    "shipping_ex_tax": 7.95,
    "subtotal_ex_tax": 29.99,
    "wrapping_ex_tax": null,
    "additional_fields": {
      "shipping_tax": 0,
      "subtotal_tax": 0,
      "tax_discount": 0,
      "wrapping_tax": null,
      "shipping_discount_ex_tax": 0,
      "subtotal_discount_ex_tax": 0
    }
  },
  "status": {
    "id": "processing",
    "name": "Processing",
    "history": [
      {
        "id": "processing",
        "name": "Processing",
        "notify": false,
        "comment": "Captured amount of €37.94 online. Transaction ID: \"66f68f2e-00ca-ad19-8e8d-321321321321\"",
        "modified_time": {
          "value": "2024-09-27T11:57:56+0100",
          "format": "Y-m-d\\\\TH:i:sO"
        }
      }
    ],
    "refund_info": null
  },
  "totals": {
    "tax": 0,
    "total": 37.94,
    "discount": 0,
    "shipping": 7.95,
    "subtotal": 29.99,
    "additional_fields": {
      "hidden_tax": 0
    }
  },
  "bundles": [],
  "comment": null,
  "refunds": [],
  "currency": {
    "id": "EUR",
    "iso3": "EUR",
    "name": "Euro",
    "rate": 1,
    "avail": true,
    "default": true,
    "symbol_left": "€",
    "symbol_right": ""
  },
  "customer": {
    "id": "",
    "email": "demo-admin@ship.town",
    "phone": "0851234567",
    "last_name": "Artur",
    "first_name": "Hanusek"
  },
  "order_id": "374180",
  "store_id": "1",
  "basket_id": "914102",
  "create_at": {
    "value": "2024-09-27T11:57:55+0100",
    "format": "Y-m-d\\\\TH:i:sO"
  },
  "discounts": [],
  "channel_id": null,
  "modified_at": {
    "value": "2024-09-27T11:57:56+0100",
    "format": "Y-m-d\\\\TH:i:sO"
  },
  "gift_message": null,
  "finished_time": null,
  "order_products": [
    {
      "name": "6ft Artificial Christmas Tree Green with Stand",
      "model": "566193",
      "price": 29.99,
      "weight": null,
      "barcode": null,
      "options": [],
      "quantity": 1,
      "tax_value": 0,
      "product_id": "80174",
      "variant_id": null,
      "tax_percent": 0,
      "total_price": 29.99,
      "weight_unit": "lb",
      "price_inc_tax": 29.99,
      "discount_amount": 0,
      "order_product_id": "1616675",
      "parent_order_product_id": null,
      "tax_value_after_discount": 0
    }
  ],
  "payment_method": {
    "name": "revolut_form",
    "additional_fields": {
      "additional_payment_info": {
        "publicId": "b8b02fca-564a-432e-ab46-8b811ec88290",
        "method_title": "Pay with Debit or Credit Card",
        "revolutOrderId": "66f6828e-00c0-ad19-8e81-b5bft6b9c30b",
        "revolutOrderData": "{\"id\":\"66f6828e-00c0-ad19-8e81-32313123131\",\"public_id\":\"b8b02fca-564a-432e-ab46-33213131213\",\"type\":\"PAYMENT\",\"state\":\"AUTHORISED\",\"created_at\":\"2024-09-27T10:57:18.714564Z\",\"updated_at\":\"2024-09-27T10:57:53.731695Z\",\"capture_mode\":\"MANUAL\",\"customer_id\":\"cfca7c19-558c-4232-b4a9-32131221321\",\"email\":\"demo-admin@ship.town\",\"phone\":\"+353851234567\",\"order_amount\":{\"value\":3794,\"currency\":\"EUR\"},\"order_outstanding_amount\":{\"value\":3794,\"currency\":\"EUR\"},\"refunded_amount\":{\"value\":0,\"currency\":\"EUR\"},\"metadata\":[],\"checkout_url\":\"https:\\\\/\\\\/checkout.revolut.com\\\\/payment-link\\\\/b8b06fca-561a-4321-ab46-3213213132131\",\"cancel_authorised_after\":\"PT2M\",\"payments\":[{\"id\":\"66f68fb0-1612-a842-b4ae-d5111111f\",\"state\":\"AUTHORISED\",\"created_at\":\"2024-09-27T10:57:52.221557Z\",\"updated_at\":\"2024-09-27T10:57:53.701741Z\",\"token\":\"45f10e51-85ed-49b7-968a-32312312313\",\"amount\":{\"value\":3794,\"currency\":\"EUR\"},\"authorised_amount\":{\"value\":3794,\"currency\":\"EUR\"},\"settled_amount\":{\"value\":3794,\"currency\":\"EUR\"},\"payment_method\":{\"id\":\"66f68fb0-ef8b-af9a-8c23-321321312131\",\"type\":\"CARD\",\"card\":{\"card_brand\":\"VISA\",\"card_bin\":\"431940\",\"funding\":\"DEBIT\",\"card_country\":\"IE\",\"card_last_four\":\"4217\",\"card_expiry\":\"11\\\\/2027\",\"cardholder_name\":\"Demo Admin\",\"checks\":{\"three_ds\":{\"state\":\"VERIFIED\",\"version\":2},\"cvv_verification\":\"MATCH\",\"address\":\"N_A\",\"postcode\":\"N_A\",\"cardholder\":\"N_A\",\"raw_cavv_result\":\"2\",\"raw_cvv_result\":\"M\",\"raw_avs_result\":\"U\"}}},\"billing_address\":{\"street_line_1\":\"34 blue set Grove\",\"region\":\"CK\",\"city\":\"Cork\",\"country_code\":\"IE\",\"postcode\":\"T1123C1\"},\"risk_level\":\"LOW\",\"fees\":[]}]}",
        "IsTransactionPending": null
      }
    }
  },
  "warehouses_ids": [],
  "billing_address": {
    "id": "744319",
    "fax": "",
    "city": "Cork",
    "type": "billing",
    "phone": "0851234567",
    "state": {
      "code": "CK",
      "name": "Co. Cork"
    },
    "gender": null,
    "region": "Co. Cork",
    "tax_id": null,
    "company": "",
    "country": {
      "name": "Ireland",
      "code2": "IE",
      "code3": "IRL"
    },
    "default": false,
    "website": null,
    "address1": "34 blue  Grove",
    "address2": "Springhill",
    "postcode": "T23C1C2",
    "last_name": "Artur",
    "first_name": "Hanusek",
    "phone_mobile": "",
    "additional_fields": {
      "prefix": "",
      "suffix": "",
      "middlename": ""
    },
    "identification_number": null
  },
  "shipping_method": {
    "name": "Order before 12pm Noon Monday to Friday (excl. Bank Holiday) for Next Working Day in Ireland - Next Working Day",
    "additional_fields": {
      "code": "flatrate_flatrate",
      "provider_code": "flatrate"
    }
  },
  "shipping_address": {
    "id": "744318",
    "fax": "",
    "city": "Cork",
    "type": "shipping",
    "phone": "085123456789",
    "state": {
      "code": "CK",
      "name": "Co. Cork"
    },
    "gender": null,
    "region": "Co. Cork",
    "tax_id": null,
    "company": "",
    "country": {
      "name": "Ireland",
      "code2": "IE",
      "code3": "IRL"
    },
    "default": false,
    "website": null,
    "address1": "34 Blue Grove",
    "address2": "Springhill",
    "postcode": "T11C1C1",
    "last_name": "Artur",
    "first_name": "Hanusek",
    "phone_mobile": "",
    "additional_fields": {
      "prefix": "",
      "suffix": "",
      "middlename": ""
    },
    "identification_number": null
  },
  "shipping_methods": [
    {
      "name": "Order before 12pm Noon Monday to Friday (excl. Bank Holiday) for Next Working Day in Ireland - Next Working Day",
      "additional_fields": {
        "code": "flatrate_flatrate",
        "provider_code": "flatrate"
      }
    }
  ],
  "additional_fields": {
    "base_currency_code": "EUR"
  },
  "order_details_url": null
}', true);
    }

    /**
     * Get InPost shipping method raw import.
     *
     * @return array
     */
    public static function getInPostRawImport($data = null): array
    {
        return json_decode($data ?? '{
  "id": "100457682",
  "total": {
    "total": 22.28,
    "total_tax": 4.16,
    "total_paid": 22.28,
    "total_discount": 0,
    "shipping_ex_tax": 10.49,
    "subtotal_ex_tax": 7.63,
    "wrapping_ex_tax": null,
    "additional_fields": {
      "shipping_tax": 2.41,
      "subtotal_tax": 1.75,
      "tax_discount": 0,
      "wrapping_tax": null,
      "shipping_discount_ex_tax": 0,
      "subtotal_discount_ex_tax": 0
    }
  },
  "status": {
    "id": "processing_zamwieniewtrakcie",
    "name": "",
    "history": [
      {
        "id": "nowy",
        "name": "Nowy",
        "notify": true,
        "comment": "",
        "modified_time": {
          "value": "2025-04-15T13:27:55+0200",
          "format": "Y-m-d\\\\TH:i:sO"
        }
      },
      {
        "id": "pending_payment",
        "name": "Oczekuje na płatność",
        "notify": false,
        "comment": "Oczekiwanie na płatność",
        "modified_time": {
          "value": "2025-04-15T13:28:05+0200",
          "format": "Y-m-d\\\\TH:i:sO"
        }
      },
      {
        "id": "processing_zamwieniewtrakcie",
        "name": "",
        "notify": true,
        "comment": "Płatność została zaakceptowana",
        "modified_time": {
          "value": "2025-04-15T13:28:49+0200",
          "format": "Y-m-d\\\\TH:i:sO"
        }
      }
    ],
    "refund_info": null
  },
  "totals": {
    "tax": 4.16,
    "total": 22.28,
    "discount": 0,
    "shipping": 10.49,
    "subtotal": 7.63,
    "additional_fields": {
      "hidden_tax": 0
    }
  },
  "bundles": [],
  "comment": null,
  "refunds": [],
  "currency": {
    "id": "PLN",
    "iso3": "PLN",
    "name": "Polish Zloty",
    "rate": 1,
    "avail": true,
    "default": true,
    "symbol_left": "PLN",
    "symbol_right": ""
  },
  "customer": {
    "id": "253694",
    "email": "anna.kowalska@example.com",
    "phone": "600123456",
    "last_name": "Kowalska",
    "first_name": "Anna"
  },
  "order_id": "593708",
  "store_id": "1",
  "basket_id": "876128",
  "create_at": {
    "value": "2025-04-15T13:27:55+0200",
    "format": "Y-m-d\\\\TH:i:sO"
  },
  "discounts": [],
  "channel_id": null,
  "modified_at": {
    "value": "2025-04-15T13:28:49+0200",
    "format": "Y-m-d\\\\TH:i:sO"
  },
  "gift_message": null,
  "finished_time": null,
  "order_products": [
    {
      "name": "Zawieszka ze stali chirurgicznej serce z księżycem i gwiazdą w kolorze złotym 19,5mm",
      "model": "ST8893B",
      "price": 3.815,
      "weight": 0,
      "barcode": null,
      "options": [],
      "quantity": 2,
      "tax_value": 1.75,
      "product_id": "35145",
      "variant_id": null,
      "tax_percent": 23,
      "total_price": 9.38,
      "weight_unit": null,
      "price_inc_tax": 4.69,
      "discount_amount": 0,
      "order_product_id": "3750004",
      "additional_fields": {
        "discount_tax_compensation": 0
      },
      "parent_order_product_id": null,
      "tax_value_after_discount": 1.75
    }
  ],
  "payment_method": {
    "name": "dialcom_przelewy",
    "additional_fields": {
      "additional_payment_info": {
        "cc_id": "",
        "cc_name": "",
        "method_id": "",
        "p24_forget": null,
        "method_name": "",
        "transaction_id": "1234567890",
        "accept_regulations": null
      }
    }
  },
  "warehouses_ids": [],
  "billing_address": {
    "id": "912294",
    "fax": "",
    "city": "Warszawa",
    "type": "billing",
    "alias": null,
    "phone": "600123456",
    "state": null,
    "gender": null,
    "region": null,
    "tax_id": null,
    "company": "",
    "country": {
      "name": "Poland",
      "code2": "PL",
      "code3": "POL"
    },
    "default": false,
    "website": null,
    "address1": "ul. Przykładowa 12/34",
    "address2": "",
    "postcode": "00-001",
    "last_name": "Kowalska",
    "first_name": "Anna",
    "phone_mobile": "",
    "additional_fields": {
      "nip": null,
      "prefix": "",
      "suffix": "",
      "middlename": ""
    },
    "identification_number": null
  },
  "shipping_method": {
    "name": " - Inpost Paczkomat 24/7 1-2 dni robocze",
    "additional_fields": {
      "code": "smpaczkomaty_smpaczkomaty",
      "provider_code": "smpaczkomaty"
    }
  },
  "shipping_address": {
    "id": "912295",
    "fax": "",
    "city": "Warszawa",
    "type": "shipping",
    "alias": null,
    "phone": "600123456",
    "state": null,
    "gender": null,
    "region": null,
    "tax_id": null,
    "company": "",
    "country": {
      "name": "Poland",
      "code2": "PL",
      "code3": "POL"
    },
    "default": false,
    "website": null,
    "address1": "ul. Przykładowa 12/34",
    "address2": "",
    "postcode": "00-001",
    "last_name": "Kowalska",
    "first_name": "Anna",
    "phone_mobile": "",
    "additional_fields": {
      "nip": null,
      "prefix": "",
      "suffix": "",
      "middlename": ""
    },
    "identification_number": null
  },
  "shipping_methods": [
    {
      "name": " - Inpost Paczkomat 24/7 1-2 dni robocze",
      "additional_fields": {
        "code": "smpaczkomaty_smpaczkomaty",
        "provider_code": "smpaczkomaty"
      }
    }
  ],
  "additional_fields": {
    "dpd_code": null,
    "enadawca": null,
    "inpost_code": null,
    "smpaczkomaty": {
      "a": 1,
      "code": "WAW01A",
      "machine_info": {
        "href": "https://api-pl-points.easypack24.net/v1/points/WAW01A",
        "name": "WAW01A",
        "type": [
          "parcel_locker"
        ],
        "agency": "WBY",
        "status": "Operating",
        "address": {
          "line1": "ul. Paczkowa 1",
          "line2": "00-001 Warszawa"
        },
        "is_next": false,
        "virtual": "0",
        "distance": null,
        "location": {
          "latitude": 52.2297,
          "longitude": 21.0122
        },
        "functions": [
          "allegro_courier_collect",
          "allegro_courier_reverse_return_send",
          "allegro_courier_send",
          "allegro_letter_reverse_return_send",
          "allegro_letter_send",
          "allegro_parcel_collect",
          "allegro_parcel_reverse_return_send",
          "allegro_parcel_send",
          "parcel",
          "parcel_collect",
          "parcel_reverse_return_send",
          "parcel_send",
          "standard_courier_reverse_return_send",
          "standard_courier_send"
        ],
        "image_url": "https://static.easypack24.net/points/pl/images/WAW01A.jpg",
        "partner_id": 0,
        "apm_doubled": null,
        "location_247": true,
        "payment_type": [
          "Payments are not supported"
        ],
        "phone_number": null,
        "location_date": null,
        "location_type": "Outdoor",
        "opening_hours": "24/7",
        "address_details": {
          "city": "Warszawa",
          "street": "Paczkowa",
          "province": "mazowieckie",
          "post_code": "00-001",
          "flat_number": null,
          "building_number": "1"
        },
        "air_index_level": null,
        "easy_access_zone": true,
        "payment_available": true,
        "payment_point_descr": "Płatność apką InPost oraz PayByLink",
        "location_description": "Przy sklepie spożywczym",
        "physical_type_mapped": "004",
        "location_description_1": null,
        "location_description_2": null,
        "operating_hours_extended": {
          "customer": null
        },
        "physical_type_description": null,
        "recommended_low_interest_box_machines_list": null
      },
      "paczkomaty_cod": 0,
      "customer_telephone": null
    },
    "p24_session_id": "100457682|3f18dcc82ca869fbd31b8d0ea8f60e1f|1",
    "base_currency_code": "PLN"
  },
  "order_details_url": null
}', true);
    }

    /**
     * Get DPD shipping method raw import.
     *
     * @return array
     */
    public static function getDpdRawImport(): array
    {
        return json_decode('{
  "id": "200154024",
  "total": {
    "total": 58.64,
    "total_tax": 10.97,
    "total_paid": 0,
    "total_discount": 0,
    "shipping_ex_tax": 9.67,
    "subtotal_ex_tax": 38,
    "wrapping_ex_tax": null,
    "additional_fields": {
      "shipping_tax": 2.23,
      "subtotal_tax": 8.74,
      "tax_discount": 0,
      "wrapping_tax": null,
      "shipping_discount_ex_tax": 0,
      "subtotal_discount_ex_tax": 0
    }
  },
  "status": {
    "id": "pending_payment",
    "name": "Oczekuje na płatność",
    "history": [
      {
        "id": "nowy",
        "name": "Nowy",
        "notify": true,
        "comment": "",
        "modified_time": {
          "value": "2025-04-15T13:43:51+0200",
          "format": "Y-m-d\\\\TH:i:sO"
        }
      },
      {
        "id": "pending_payment",
        "name": "Oczekuje na płatność",
        "notify": false,
        "comment": "Oczekiwanie na płatność",
        "modified_time": {
          "value": "2025-04-15T13:44:01+0200",
          "format": "Y-m-d\\\\TH:i:sO"
        }
      }
    ],
    "refund_info": null
  },
  "totals": {
    "tax": 10.97,
    "total": 58.64,
    "discount": 0,
    "shipping": 9.67,
    "subtotal": 38,
    "additional_fields": {
      "hidden_tax": 0
    }
  },
  "bundles": [],
  "comment": null,
  "refunds": [],
  "currency": {
    "id": "PLN",
    "iso3": "PLN",
    "name": "Polish Zloty",
    "rate": 1,
    "avail": true,
    "default": true,
    "symbol_left": "PLN",
    "symbol_right": ""
  },
  "customer": {
    "id": "220832",
    "email": "anna.kowalska@example.com",
    "phone": "600123456",
    "last_name": "Kowalska",
    "first_name": "Anna"
  },
  "order_id": "593711",
  "store_id": "2",
  "basket_id": "870904",
  "create_at": {
    "value": "2025-04-15T13:43:50+0200",
    "format": "Y-m-d\\\\TH:i:sO"
  },
  "discounts": [],
  "channel_id": null,
  "modified_at": {
    "value": "2025-04-15T13:44:01+0200",
    "format": "Y-m-d\\\\TH:i:sO"
  },
  "gift_message": null,
  "finished_time": null,
  "order_products": [
    {
      "name": "Hematyt platerowany kulka gładka light silver satine 8-9mm",
      "model": "KA3332",
      "price": 14,
      "weight": 0,
      "barcode": null,
      "options": [],
      "quantity": 1,
      "tax_value": 3.22,
      "product_id": "19100",
      "variant_id": null,
      "tax_percent": 23,
      "total_price": 17.22,
      "weight_unit": null,
      "price_inc_tax": 17.22,
      "discount_amount": 0,
      "order_product_id": "3750063",
      "additional_fields": {
        "discount_tax_compensation": 0
      },
      "parent_order_product_id": null,
      "tax_value_after_discount": 3.22
    },
    {
      "name": "Hematyt kulka gładka light gold 2,5mm",
      "model": "KA9666",
      "price": 11,
      "weight": 0,
      "barcode": null,
      "options": [],
      "quantity": 1,
      "tax_value": 2.53,
      "product_id": "40080",
      "variant_id": null,
      "tax_percent": 23,
      "total_price": 13.53,
      "weight_unit": null,
      "price_inc_tax": 13.53,
      "discount_amount": 0,
      "order_product_id": "3750064",
      "additional_fields": {
        "discount_tax_compensation": 0
      },
      "parent_order_product_id": null,
      "tax_value_after_discount": 2.53
    },
    {
      "name": "Hematyt kulka gładka light gold 3mm",
      "model": "KA9659",
      "price": 13,
      "weight": 0,
      "barcode": null,
      "options": [],
      "quantity": 1,
      "tax_value": 2.99,
      "product_id": "40073",
      "variant_id": null,
      "tax_percent": 23,
      "total_price": 15.99,
      "weight_unit": null,
      "price_inc_tax": 15.99,
      "discount_amount": 0,
      "order_product_id": "3750065",
      "additional_fields": {
        "discount_tax_compensation": 0
      },
      "parent_order_product_id": null,
      "tax_value_after_discount": 2.99
    }
  ],
  "payment_method": {
    "name": "dialcom_przelewy",
    "additional_fields": {
      "additional_payment_info": {
        "cc_id": "",
        "cc_name": "",
        "method_id": "",
        "p24_forget": null,
        "method_name": "",
        "accept_regulations": null
      }
    }
  },
  "warehouses_ids": [],
  "billing_address": {
    "id": "912300",
    "fax": "",
    "city": "Warszawa",
    "type": "billing",
    "alias": null,
    "phone": "600123456",
    "state": null,
    "gender": null,
    "region": null,
    "tax_id": null,
    "company": "",
    "country": {
      "name": "Poland",
      "code2": "PL",
      "code3": "POL"
    },
    "default": false,
    "website": null,
    "address1": "ul. Przykładowa 12/34",
    "address2": "",
    "postcode": "00-001",
    "last_name": "Kowalska",
    "first_name": "Anna",
    "phone_mobile": "",
    "additional_fields": {
      "nip": null,
      "prefix": "",
      "suffix": "",
      "middlename": ""
    },
    "identification_number": null
  },
  "shipping_method": {
    "name": "DPD ODBIÓR W PUNKCIE-ŻABKA / AUTOMATY - DPD ODBIÓR W PUNKCIE-ŻABKA / AUTOMATY",
    "additional_fields": {
      "code": "dpd5_dpd5",
      "provider_code": "dpd5"
    }
  },
  "shipping_address": {
    "id": "912301",
    "fax": "",
    "city": "Warszawa",
    "type": "shipping",
    "alias": null,
    "phone": "600123456",
    "state": null,
    "gender": null,
    "region": null,
    "tax_id": null,
    "company": "",
    "country": {
      "name": "Poland",
      "code2": "PL",
      "code3": "POL"
    },
    "default": false,
    "website": null,
    "address1": "ul. Przykładowa 12/34",
    "address2": "",
    "postcode": "00-001",
    "last_name": "Kowalska",
    "first_name": "Anna",
    "phone_mobile": "",
    "additional_fields": {
      "nip": null,
      "prefix": "",
      "suffix": "",
      "middlename": ""
    },
    "identification_number": null
  },
  "shipping_methods": [
    {
      "name": "DPD ODBIÓR W PUNKCIE-ŻABKA / AUTOMATY - DPD ODBIÓR W PUNKCIE-ŻABKA / AUTOMATY",
      "additional_fields": {
        "code": "dpd5_dpd5",
        "provider_code": "dpd5"
      }
    }
  ],
  "additional_fields": {
    "dpd_code": "PL68030",
    "enadawca": null,
    "inpost_code": null,
    "smpaczkomaty": {
      "code": null,
      "paczkomaty_cod": 0,
      "customer_telephone": null
    },
    "p24_session_id": "200154024|example_session|2",
    "base_currency_code": "PLN"
  },
  "order_details_url": null
}', true);
    }
}
