...
class DpdPolandDropOffService extends ShippingServiceAbstract
{
    public function ship(int $order_id, ?ShippingService $shippingService = null): Collection
    {
        ...
        if (!$order->shippingAddress->locker_box_code) {
            throw new ShippingServiceException('Locker box code is required.');
        }
        ...
        try {
            $shippingLabels = ShippyProApi::createShippingLabel($order, [
                'Params' => [
                    ...
                    "CarrierOptions" => [
                        [
                            "name" => "dpdpolandpointid",
                            "value" => $order->shippingAddress->locker_box_code
                        ]
                    ],
                    'parcels' => [
                        [
            ...
            throw new ShippingServiceException('DPD Poland (Drop-Off): ' . $exception->getMessage());
        }
    }
}
