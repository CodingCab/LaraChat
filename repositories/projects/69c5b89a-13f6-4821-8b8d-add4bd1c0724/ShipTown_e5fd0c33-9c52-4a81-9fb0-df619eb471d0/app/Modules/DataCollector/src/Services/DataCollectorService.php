...
use App\Modules\DataCollector\src\Jobs\ImportAsSaleJob;
...
class DataCollectorService
{
    ...
    public static function runAction(DataCollection $dataCollection, $action): void
    {
        ...
        if ($action === 'import_as_sale') {
            $dataCollection->update([
                'type' => DataCollectionTransaction::class,
                'currently_running_task' => ImportAsSaleJob::class,
            ]);

            $dataCollection->delete();

            ImportAsSaleJob::dispatch($dataCollection->id);

            return;
        }

        if ($action === 'update_next_transaction_number') {
            $transactionNumber = PointOfSaleConfiguration::query()->first();
            $dataCollection->update([
                'name' => 'Transaction #' . $transactionNumber->next_transaction_number,
            ]);
            $transactionNumber->increment('next_transaction_number');
        ...
        $sourceDataCollection->records()
            ...
            ->each(function (DataCollectionRecord $sourceRecord) use ($destinationDataCollection) {
                ...
                DataCollectionRecord::query()->firstOrCreate([
                    ...
                    'sales_tax_code' => $sourceRecord->product->default_tax_code,
                ]);
            });
    ...
    public static function importAsSale(DataCollectionRecord $record): void
    {
        DB::transaction(function () use ($record) {
            $record->refresh();

            $quantitySold = $record->quantity_scanned;
            ...
            InventoryService::sell($inventory, $quantitySold * (-1), [
                'occurred_at' => $record->dataCollection->deleted_at ?? now()->utc()->toDateTimeLocalString(),
                'description' => Str::substr($record->dataCollection->name, 0, 50),
                'custom_unique_reference_id' => $custom_unique_reference_id,
            ]);

            $record->update([
                'total_transferred_out' => $record->total_transferred_out - $quantitySold,
                'quantity_scanned' => 0,
            ]);
        });
    }
    ...
    public static function splitRecord(DataCollectionRecord $record, mixed $quantityToExtract): void
    {
        ...
        $newRecord = DataCollectionRecord::firstOrCreate(
            ...
            [
                ...
                'sales_tax_code' => $record->product->default_tax_code,
            ]
        );
