...
class CsvImportController extends Controller
{
    ...
    public function store(Request $request): JsonResource
    {
        ...
        // update existing records within the data collection
        DB::statement('
            UPDATE data_collection_records
            JOIN tempTable_csv_import AS tempTable
                ON tempTable.product_id = data_collection_records.product_id
                AND data_collection_records.data_collection_id = '.$validatedData['data_collection_id'].'
            SET data_collection_records.quantity_requested = data_collection_records.quantity_requested + IFNULL(tempTable.quantity_requested, 0),
                data_collection_records.quantity_scanned = data_collection_records.quantity_scanned + IFNULL(tempTable.quantity_scanned, 0)
        ');

        // insert new records for products not yet present in the data collection
        DB::statement('
            ...
            LEFT JOIN data_collection_records AS existing
                ON existing.data_collection_id = '.$validatedData['data_collection_id'].'
                AND existing.product_id = tempTable.product_id
