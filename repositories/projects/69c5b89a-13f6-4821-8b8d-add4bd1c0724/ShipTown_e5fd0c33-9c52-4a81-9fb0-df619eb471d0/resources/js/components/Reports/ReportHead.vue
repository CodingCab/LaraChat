            ...
            <div class="flex-grow-1">
                <div class="filter-container d-none d-lg-flex" ref="filterContainer">
                    <p class="text-primary small" v-for="filter in urlFilters">
                        <span v-html="urlFiltersToHumanString(filter)" @click.prevent="editFilter(filter)"></span>
                ...
                <a href="#" @click.prevent="showFilters = !showFilters" class="float-right d-lg-none small">
                    {{ showFilters ? $t('HIDE') : $t('FILTERS') }} <span v-show="!showFilters">({{ urlFilters.length }})</span>
                </a>
            </div>
        </div>

        <div class="filter-container d-flex d-lg-none" ref="filterContainer" v-if="showFilters || autoExpandFilters" >
          <p class="text-primary small d-flex justify-content-end align-items-center" v-for="filter in urlFilters">
            <span class="text-right" v-html="urlFiltersToHumanString(filter)" @click.prevent="editFilter(filter)"></span>
            <button @click="removeFilter(filter)" class="btn btn-link p-0 ml-1 mb-1">x</button>
          </p>
        </div>
    </div>
...
export default {
    ...
    methods: {
        readFilters() {
            this.urlFilters = Object.keys(this.$router.currentRoute.query)
                .filter(aFilter => aFilter.startsWith('filter['));
        },

        removeFilter(filter) {
          this.removeUrlParameter(filter);
        },
        ...
        urlFiltersToHumanString(urlParameter) {
          ...
          const filterOperatorTranslated = this.$t(filterOperator);
          return [fieldNameFormatted, filterOperatorTranslated, filterValueFormatted].join(' ');
        },
    },
}
</script>
