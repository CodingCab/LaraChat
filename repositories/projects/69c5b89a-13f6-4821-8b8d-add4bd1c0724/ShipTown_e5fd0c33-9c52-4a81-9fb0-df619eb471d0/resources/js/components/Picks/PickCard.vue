                    ...
                    <div class="col p-2 pl-3 rounded">
                        <div class="row">
                            <div class="col-md-8 col-lg-6">
                                ...
                                <div>{{ $t('sku') }}: <div dusk="product_sku" id="product_sku" class="font-weight-bold d-inline"> {{ pick['sku_ordered'] }} </div></div>
                                <div>{{ $t('product') }}: <b> <a :href="'/products?hide_nav_bar=true&filter[sku]=' + product_sku" @click.prevent="showProductDetailsModal">{{ product_sku }}</a></b></div>
                            </div>
                            <div class="col-md-4 col-lg-6">
                                <div class="row">
                                    <div class="col-5 small" >
                                        <div v-bind:class="{ 'bg-warning': Number(pick['inventory_source_quantity']) === 0 }">
                                            {{ $t('in stock') }}: <b> {{ Number(pick['inventory_source_quantity']) }} </b>
                                        </div>
                                        <p class="">{{ $t('orders') }}: {{ pick.order_product_ids.length }}</p>
                                    </div>
                                    <div class="col-3 text-center" >
                                        <small>{{ $t('to pick') }}</small>
                                        <h3 class="m-auto" v-bind:class="{ 'bg-warning': Number(pick['total_quantity_to_pick']) !== 1 }">{{ Number(this.pick['total_quantity_to_pick']) }}</h3>
                                    </div>
