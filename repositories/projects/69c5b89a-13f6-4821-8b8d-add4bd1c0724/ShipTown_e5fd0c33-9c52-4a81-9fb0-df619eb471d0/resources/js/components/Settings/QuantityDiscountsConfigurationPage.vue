    ...
    <div>
        ...
        <search-and-option-bar :isStickable="true">
            <barcode-input-field :input_id="'barcode_input'" :placeholder="$t('Search')" ref="barcode" :url_param_name="'search'"
                @refreshRequest="reloadDiscountsList" @barcodeScanned="findText" />
        ...
        <template v-if="isLoading === false && discounts !== null && Array.isArray(discounts) && discounts.length === 0">
        ...
        <template v-if="discounts">
            <swiping-card v-for="discount in discounts" :key="discount.id" disable-swipe-right disable-swipe-left>
...
export default {
    ...
    data: () => ({
        ...
        discountTypes: {
            ...
            'App\\Modules\\DataCollectorQuantityDiscounts\\src\\Jobs\\BuyXForYPercentDiscount': this.$t('Buy X get Y percent discount'),
            'App\\Modules\\DataCollectorQuantityDiscounts\\src\\Jobs\\VolumePurchasePercentDiscount': this.$t('Volume Purchase Percent Discount'),
            'App\\Modules\\DataCollectorQuantityDiscounts\\src\\Jobs\\VolumePurchasePriceDiscount': this.$t('Volume Purcahse Price Discount')
        },
    }),
