        ...
        <div class="modal-dialog modal-dialog-centered">
            <div ref="loadingContainer2" class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ $t('Edit Order Status') }}</h5>
                </div>
                <div class="modal-body">
                    <ValidationObserver ref="form">
                            ...
                            <div class="form-group row">
                                <label class="col-sm-3 col-form-label" for="order_on_hold">{{ $t('Order On Hold') }}</label>
                                <div class="col-sm-9">
                            ...
                            <div class="form-group row">
                                <label class="col-sm-3 col-form-label" for="hidden">{{ $t('Hidden') }}</label>
                                <div class="col-sm-9">
                ...
                <div class="modal-footer" style="justify-content:space-between">
                    <button type="button" @click="confirmDelete" class="btn btn-outline-danger float-left">{{ $t('Archive') }}</button>
                    <div>
    ...
    methods: {
        submit() {
            ...
            this.apiPutOrderStatus(this.orderStatus.id, {
                ...
                .then(({ data }) => {
                    this.$snotify.success(this.$t('Order status updated.'));
        ...
        confirmDelete() {
            if(this.reservesStock || this.orderActive || this.syncEcommerce || this.syncEcommerce) {
              this.showError(this.$t('Please disable all options before archiving'));
              return;
            }

            if(this.isModified) {
              this.showError(this.$t('Please save changes before archiving'));
              return;
            }

            this.$snotify.confirm(this.$t(`Do you want to archive this status?`),  {
        ...
        delete(id, index) {
            ...
            this.apiDeleteOrderStatus(id)
                .then(() => {
                    ...
                    this.$snotify.success(this.$t('Order status archived.'));
                })
                .catch(error => {
                    if (error.response) {
