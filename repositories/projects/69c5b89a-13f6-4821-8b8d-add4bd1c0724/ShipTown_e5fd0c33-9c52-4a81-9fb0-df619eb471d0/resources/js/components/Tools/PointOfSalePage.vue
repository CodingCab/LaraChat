<template>
    <div>
        ...
        <div v-else class="m-auto" style="width: 450px">
            <h1>{{ $t('Point of Sale') }}</h1>
            <input type="text" placeholder="Username" class="form-control mt-2">
            <input type="password" placeholder="Password" class="form-control mt-2">
            <button id="start-new-transaction-button" @click="startNewTransaction" class="btn btn-primary form-control mt-2">{{ $t('Start Transaction') }}</button>
        </div>
    </div>
</template>
<script>
