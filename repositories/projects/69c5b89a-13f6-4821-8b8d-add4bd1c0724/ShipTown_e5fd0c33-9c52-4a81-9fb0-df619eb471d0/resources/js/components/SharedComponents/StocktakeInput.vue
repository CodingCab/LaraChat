<template>
    <div>
        <barcode-input-field v-if="this.currentUser()['warehouse_id']"
            :input_id="input_id"
            :placeholder="this.currentUser()['warehouse_id'] === null ? $t('Select warehouse to enable Stocktaking function') : $t('Scan product to stocktake')"
        ...
        >
            <template v-if="inventory">
                ...
                <div class="small" v-bind:class="{ 'bg-warning': inventory['last_counted_at'] !== null && isCountedRecently(inventory['last_counted_at'], 28)}">{{ $t('last counted') }}: <strong>{{ formatDateTime(inventory['last_counted_at']) }}</strong></div>
                <div class="small">stock: {{ inventory.quantity }}</div>
                <div class="row mt-2">
                    <div class="col-6">
                        <label class="small text-lowercase" for="adjust-by-request-input">{{ $t('Adjust by') }}</label>
                        <input class="form-control " :placeholder="$t('Adjust by')" :class="{ 'border-danger': this.adjustByQuantity < 0, 'border-success': this.adjustByQuantity > 0}"
                    ...
                    <div class="col-6">
                        <label class="small text-lowercase" for="adjust-by-request-input">{{ $t('New quantity') }}</label>
                        <input class="form-control" :placeholder="$t('New quantity')" :class="{ 'border-danger': this.adjustByQuantity < 0, 'border-success': this.adjustByQuantity > 0}"
                ...
                <input class="form-control mt-2" :placeholder="$t('Reason')" disabled
            ...
            <template #modal-footer="{ ok, cancel }">
                <b-button style="width: 80px" class="btn" variant="secondary" @click="cancel()">{{ $t('Cancel') }}</b-button>
                <b-button style="width: 80px" class="btn" variant="primary" @click="ok()">{{ $t('Update') }}</b-button>
            </template>
        </b-modal>
    </div>
</template>
