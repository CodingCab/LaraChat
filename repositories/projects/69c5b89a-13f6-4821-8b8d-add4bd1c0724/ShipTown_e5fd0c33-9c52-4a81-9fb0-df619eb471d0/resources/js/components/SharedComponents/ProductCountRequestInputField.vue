...
<!--                    <number-card label="requested" :number="requestedQuantity"></number-card>-->
                ...
                <div class="row">
                    <div class="col-12">
                        <input class="form-control" :placeholder="$t('quantity to add')" :class="{ 'border-danger': this.quantity < 0, 'border-success': this.quantity > 0}"
                ...
                this.apiGetProducts(params)
                    .then(response => {
                        if (response.data.data.length === 0) {
                            this.notifyError(this.$t('No product found with barcode ') + barcode);
                            return;
                        }
