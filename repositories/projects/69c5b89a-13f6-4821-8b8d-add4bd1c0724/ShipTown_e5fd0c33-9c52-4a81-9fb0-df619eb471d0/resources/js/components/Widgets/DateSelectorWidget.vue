    ...
    <div class="text-right">
        <div class="dropdown">
            ...
            <div class="dropdown-menu" aria-labelledby="dropdownDateRange">
                <a class="dropdown-item" @click.prevent="applyFilter('today,now')">{{ $t('Today') }}</a>
                <a class="dropdown-item" @click.prevent="applyFilter('yesterday,today')">{{ $t('Yesterday') }}</a>
                <a class="dropdown-item" @click.prevent="applyFilter('-7days,now')">{{ $t('Last 7 days') }}</a>
                <a class="dropdown-item" @click.prevent="applyFilter('this week monday,now')">{{ $t('This week') }}</a>
                <a class="dropdown-item" @click.prevent="applyFilter('last week monday,this week monday')">{{ $t('Last Week') }}</a>
                <a class="dropdown-item" dusk="custom-date-link" @click.prevent="$bvModal.show('modal-date-selector-widget')">{{ $t('Custom Date') }}</a>
            </div>
        </div>
    </div>

        <ModalDateBetweenSelector
        :from_datetime.sync="starting_date"
        :to_datetime.sync="ending_date"
...
export default {
    ...
    watch: {
        starting_date(val){
            this.formatted_starting_date = moment(val).format('YYYY-MM-DD hh:mm')
        },
        ending_date(val){
            this.formatted_ending_date = moment(val).format('YYYY-MM-DD hh:mm')
        }
    },
    methods: {
        applyFilter(range){
        ...
        onModalApply(event){
            this.starting_date = event.from_datetime
            this.ending_date = event.to_datetime
            this.validateFilter()
        },
    }
}
