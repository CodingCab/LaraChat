...
const runCommand = async () => {
    ...
    try {
        ...
        if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
            commandResult.value = 'Command timed out. The command is taking longer than expected. For long-running commands like git clone, try running them in smaller steps or check if the process is still running on the server.';
        } else if (error.response?.status === 504) {
            commandResult.value = 'Gateway timeout: The server took too long to respond. This usually happens with long-running commands. The command might still be running on the server.';
            ...
            <div class="flex flex-col gap-4 mb-4">
                <div class="flex flex-col gap-2">
                    ...
                    <div v-if="commandResult" 
                         :class="[
                             'p-4 rounded-md text-sm transition-all font-mono',
                             commandSuccess 
                        ...
                        <div class="flex items-start gap-3">
                            ...
                            <div class="flex-1">
                                <p class="font-medium mb-2">{{ commandSuccess ? 'Command executed successfully' : 'Command failed' }}</p>
                                <pre class="whitespace-pre-wrap break-words overflow-x-auto">{{ commandResult }}</pre>
                            </div>
                        </div>
                    </div>
                </div>
